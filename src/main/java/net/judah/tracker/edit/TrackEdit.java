...
public abstract class TrackEdit extends JPanel implements ActionListener {
	...
	private void genericButtons() {
		...
		JLabel trklbl = new JLabel(track.getName(), JLabel.CENTER);
        trklbl.setFont(Constants.Gui.BOLD);

		fillPatterns();
		MouseAdapter rename = new MouseAdapter() {
        ...
        pattern.addMouseListener(rename);
	...
	private JPanel playRecMpk() {
        ...
        if (track.isSynth()) {
			mpk.addActionListener(e -> {
				((PianoTrack)track).setLatch(!((PianoTrack)track).isLatch());
				((SynthTracks)track.getTracker()).checkLatch();
			});
        }		
	...
	private void createTrackNav() {
        ...
        chBack.addActionListener(e -> JudahZone.getTracker().next(false));
        Click chNext = new Click(">");
        chNext.addActionListener(e -> JudahZone.getTracker().next(true));
