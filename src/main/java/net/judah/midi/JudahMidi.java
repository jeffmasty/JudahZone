...
//        }
        ...
        JudahClock.setClockOut(clockOut);
            ...
            for (BeatBox b : clock.getSequencers()) {
                ...
                while (poll != null) {
                    JackMidi.eventWrite(b.getMidiOut(),
                            ticker(), poll.getMessage(), poll.getLength());
                	poll = b.getQueue().poll();
                }
            }
