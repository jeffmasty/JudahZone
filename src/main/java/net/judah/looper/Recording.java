...
public class Recording extends Vector<float[][]> {
	...
	@Getter private static final long sampleRate = Constants.sampleRate();	
	...
	public static Recording load(String file) throws Exception {
		return load(new File(Constants.SAMPLES, file));
	}
	
	public static Recording load(File file) throws Exception {
		Recording result = openWavFile(file);
		...
		do {
            ...
            framesRead = result.readFrames(buffer, 0, frameSize);
	...
	private static Recording openWavFile(File file) throws Exception {
		Recording wavFile = new Recording(0, false);
		wavFile.file = file;
		...
		while (true)
		{
			...
			if (chunkID == FMT_CHUNK_ID)
			{
				...
				long srate = getLE(wavFile.buffer, 4, 4);
				if (srate != sampleRate) 
					throw new Exception("Sample rate(" + sampleRate + ") vs: " + srate + " " + file.getAbsolutePath());
				
				// wavFile.sampleRate = getLE(wavFile.buffer, 4, 4);
				wavFile.blockAlign = (int) getLE(wavFile.buffer, 12, 2);
				
				long bits = getLE(wavFile.buffer, 14, 2);
				if (bits != validBits)
		...
		wavFile.floatScale = 1 << (validBits - 1);
