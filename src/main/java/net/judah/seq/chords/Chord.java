...
public class Chord extends ArrayList<Key> {
	...
	public Chord(String parse) {
		...
		caret = 0;
		if ( (suffix.startsWith("m") && false == suffix.startsWith("maj")) || suffix.startsWith("-")) {
			minor(key);
			caret += 1;
		}
		...
		if (suffix.substring(caret, caret + 1).equals("7")) {
			...
			String maj7 = suffix.substring(caret);
			if (maj7.startsWith("▲7")) {
				major7();
				caret += 2;
			}
			else if (maj7.startsWith("maj7")) {
				major7();
				chord = chord.replace("maj7", "▲7");
				caret += 4;
			}
