...
public class DrumZone extends HiringAgency implements Floating {
	...
	public DrumZone(TrackList<DrumTrack> list) {
	    ...
	    // TabZone.edit(tracks.getCurrent()); // ensure global editor is set to initial track
	...
	void setCurrent(MidiTrack track) { // fra-gee-leh
		tracks.setCurrent(track);
		if (TabZone.getInstance() != null)
			TabZone.edit(track); // update global editor/undo target when current track changes
	}
