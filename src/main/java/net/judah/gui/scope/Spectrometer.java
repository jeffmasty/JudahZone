...
public class Spectrometer extends JPanel implements Closeable, Floating, Gui.Mouser {
	...
	public JComponent getControls() {
        JSlider dampen = new JSlider(0, 100, 50); // sensitivity: 0 (strict) .. 100 (show many)
    	JSlider yScale = new JSlider(1, 100, 50);  // height multiplier control (logarithmic)
    	dampen.addChangeListener(l->setSensitivity(dampen.getValue()));
    	yScale.addChangeListener(l->setYScale(yScale.getValue()));
        return Gui.box(new JLabel(" Scale "), Gui.resize(yScale, Size.SMALLER),
        		new JLabel(" Wet "), Gui.resize(dampen, Size.SMALLER));
    }
    ...
    private void drawImage(Transform t) {
        ...
        // 4) compute sensitivity threshold from dampen slider
