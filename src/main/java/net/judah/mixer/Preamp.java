...
/** Preamp UI component with logarithmic mapping between a slider (1..100)
 *  and a preamp range (MIN..MAX). Exposes get/set for integration with models
 *  and table cell editors/renderers. */
...
public void setPreamp(float preamp) {
    ...
    if (pct < slider.getMinimum()) pct = slider.getMinimum();
    if (pct > slider.getMaximum()) pct = slider.getMaximum();
    slider.setValue(pct);
    updateLabelFromSlider();
}
...
/* Replace the existing PreampCellRenderer in ChannelList.java with this implementation.
   Renders an always-visible preamp slider per row (disabled for non-editing state). */
public static class Renderer implements TableCellRenderer {
    ...
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
        ...
        if (isSelected) preampComp.setBackground(table.getSelectionBackground());
        else preampComp.setBackground(table.getBackground());
...
/* Editor: editable Preamp component embedded in the cell
   Now defers updating the table/model until editing stops to avoid noisy autosaves. */
public static class Editor extends AbstractCellEditor implements TableCellEditor {
    ...
    Editor(CustomTableModel model, Runnable onStop) {
        ...
        preampComp.addChangeListener(e -> {
            if (editingRow < 0 || internalChange) return;
            dirty = true;
    ...
    public boolean stopCellEditing() {
        if (editingRow >= 0 && dirty) {
            ...
            for (int i = 0; i < model.getColumnCount(); i++)
                if ("Preamp".equalsIgnoreCase(model.getColumnName(i))) { col = i; break; }
            if (col == -1) col = 5; // fallback consistent with previous layout
